export function TidyPath(path, convertAfterQuery = false) {
    path = (path ? path.trim() : '');
    if (!path) {
        return '';
    }
...
export function PathToRelative(path, origin, prefix, convertAfterQuery = false) {
...
export function JoinPath({ base, query }, origin, prefix, prependOrigin, convertAfterQuery = false) {
