...
export const FormDirectiveHandler = CreateDirectiveHandlerCallback(FormDirectiveName, ({ componentId, component, contextElement, expression, argKey, argOptions }) => {
    ...
    if (argKey === 'field') {
        return EvaluateLater({ componentId, contextElement, expression })((value) => {
            var _a;
            if (IsObject(value)) {
                let proxy = (_a = FindComponentById(componentId)) === null || _a === void 0 ? void 0 : _a.FindElementLocalValue(contextElement, localKey, true);
    ...
    if (argKey in FormMiddlewares) { //Bind data
        ...
        UseEffect({ componentId, contextElement,
            callback: () => evaluate((data) => {
                var _a;
                let component = FindComponentById(componentId), proxy = component === null || component === void 0 ? void 0 : component.FindElementLocalValue(contextElement, localKey, true);
                if (proxy) { //Bind data
    ...
    let id = resolvedComponent.GenerateUniqueId('form_proxy_'), middlewares = new Array(), options = ResolveOptions({
        options: {
            refresh: false,
            reload: false,
            persistent: false,
    ...
    let evaluate = EvaluateLater({ componentId, contextElement, expression }), doEvaluation = (ok, data) => {
    ...
    let afterHandledEvent = (ok, data) => {
        ...
        if ((!options.success && !options.error) || (options.success && ok) || (options.error && !ok)) {
            if (options.nexttick) {
                (_a = FindComponentById(componentId)) === null || _a === void 0 ? void 0 : _a.GetBackend().changes.AddNextTickHandler(() => doEvaluation(ok, data));
            }
            else {
        ...
        GetGlobal().GetFetchConcept().Get(url, info).then(res => res.json()).then((response) => {
            ...
            JournalTry(() => {
                ...
                if (!options.silent && (response.hasOwnProperty('alert') || response.hasOwnProperty('report'))) {
                    (_a = GetGlobal().GetConcept('alert')) === null || _a === void 0 ? void 0 : _a.Notify(response['alert'] || response['report']);
                }
                afterHandledEvent((response['ok'] !== false), response['data']);
                if (response['ok'] === false) {
                    return;
                }
                let router = GetGlobal().GetConcept(RouterConceptName), after = null;
                if (response.hasOwnProperty('redirect')) {
                    ...
                    form.reset();
                    (_b = FindComponentById(componentId)) === null || _b === void 0 ? void 0 : _b.GetBackend().changes.AddNextTickHandler(() => form === null || form === void 0 ? void 0 : form.dispatchEvent(new CustomEvent(`${FormDirectiveName}.reset`)));
                }
                if (after) {
                    after();
                }
    ...
    (_a = resolvedComponent.FindElementScope(contextElement)) === null || _a === void 0 ? void 0 : _a.SetLocal(localKey, CreateInplaceProxy(BuildGetterProxyOptions({
        getter: (prop) => {
            var _a;
...
export function AddFormMiddleware(middleware) {
    FormMiddlewares[middleware.GetKey()] = middleware;
}
export function RemoveFormMiddleware(name) {
