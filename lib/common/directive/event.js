...
function ForwardEvent(component, contextElement, event, expression, options) {
    let name = (0, get_1.GetGlobal)().GetConfig().GetDirectiveName('on'), joinedOptions = (options || []).join('.');
    let qName = (joinedOptions ? `${name}:${event}.${joinedOptions}` : `${name}:${event}`), directive = (0, create_1.CreateDirective)(qName, (expression || ''));
