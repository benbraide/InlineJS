...
function ProcessDirectives({ component, element, options = {}, proxyAccessHandler }) {
    ...
    if (resolvedComponent.IsDestroyed()) {
        (0, error_1.JournalError)('Component is destroyed.', 'InlineJS.ProcessDirectives', element);
        return false;
    }
    let repeats = 0;
    ...
    !(0, template_1.IsTemplate)(element) && (0, get_1.GetGlobal)().DispatchTextContentProcessing({
        componentId: resolvedComponent.GetId(),
        component: resolvedComponent,
        contextElement: element,
        proxyAccessHandler,
    });
    if ((0, is_custom_element_1.IsCustomElement)(element)) { // Element is defined as custom
