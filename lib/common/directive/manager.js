...
function CreateComposedDirectiveHandler(originalHandler) {
    const composition = {
        handler: (_a) => {
            var { argKey } = _a, rest = __rest(_a, ["argKey"]);
            // Call extension if it exists
            if (composition.extensions.hasOwnProperty(argKey)) {
                composition.extensions[argKey](Object.assign({ argKey }, rest));
            }
            else { // Pass to main handler
                originalHandler(Object.assign({ argKey }, rest));
            }
        },
        extensions: {},
    };
...
class DirectiveManager {
    ...
    AddExpansionRule(rule) {
        const id = (0, get_1.GetGlobal)().GenerateUniqueId('exrule_');
